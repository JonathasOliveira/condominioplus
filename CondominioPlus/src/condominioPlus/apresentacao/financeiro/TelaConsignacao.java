 ...
 * @author eugenia
 */
public class TelaConsignacao extends javax.swing.JInternalFrame {
    ...
    /** Creates new form TelaConsignacao */
    public TelaConsignacao(Condominio condominio) {

        this.condominio = condominio;
        if (condominio.getConsignacao() == null) {
            consignacao = new Consignacao();
            condominio.setConsignacao(consignacao);
            consignacao.setCondominio(condominio);
            new DAO().salvar(condominio);
        } else {
            consignacao = condominio.getConsignacao();
            if (consignacao.getCondominio() == null) {
                consignacao.setCondominio(condominio);
                new DAO().salvar(condominio);
            }
        }
        ...
        if (condominio != null) {
            this.setTitle("Consignação - " + condominio.getRazaoSocial());
        }
    }

    public void carregarTabela() {

        modeloTabela = new TabelaModelo_2<Pagamento>(tabelaConsignacao, "Data, Histórico, Valor, Saldo, Conta, Tipo".split(",")) {
        ...
        tabelaConsignacao.getColumn(modeloTabela.getCampo(2)).setCellRenderer(direita);
        tabelaConsignacao.getColumn(modeloTabela.getCampo(3)).setCellRenderer(direita);

    }

    private List<Pagamento> getPagamentos() {
        pagamentos = new DAO().listar("PagamentosConsignacao", condominio.getConsignacao());
    ...
    private void verificarListaVaziaConsignacao(Pagamento p2) {
    ...
    private void verificarLista() {
        if (condominio.getConsignacao().getPagamentos().size() == 1) {
    ...
    private void preencherObjeto() {
        if (conta.getNomeVinculo().equals("CO")) {
            ...
            verificarListaVaziaConsignacao(pagamento);
            ...
            ApresentacaoUtil.exibirAdvertencia("Selecione uma conta vinculada à Consignação!", this);
            return;
        }
    ...
    private void apagarItensSelecionados() {
        ...
        if (modeloTabela.getLinhaSelecionada() > -1) {
            ...
            List<Pagamento> itensRemoverConsignacao = modeloTabela.getObjetosSelecionados();
            List<Pagamento> itensRelacionadosRemover = new ArrayList<Pagamento>();

            for (Pagamento p : itensRemoverConsignacao) {
            ...
            new DAO().remover(itensRemoverConsignacao);
            condominio.getConsignacao().getPagamentos().removeAll(itensRemoverConsignacao);
            new DAO().salvar(condominio);
    ...
    private class ControladorEventos extends ControladorEventosGenerico {
        ...
        public void configurar() {
            ...
            tabelaConsignacao.addMouseListener(this);
                ...
                if (new LimitarCaracteres(10).ValidaNumero(txtConta)) {
                    if (!txtConta.getText().equals("") && txtConta.getText() != null) {
                        ...
                        if (resultado != null) {
                            ...
                            ApresentacaoUtil.exibirErro("Código Inexistente!", TelaConsignacao.this);
                            txtConta.setText("");
                            txtConta.grabFocus();
                            return;
                        }
    ...
    private void initComponents() {
        ...
        tabelaConsignacao.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jScrollPane1.setViewportView(tabelaConsignacao);
    ...
    private javax.swing.JTable tabelaConsignacao;
